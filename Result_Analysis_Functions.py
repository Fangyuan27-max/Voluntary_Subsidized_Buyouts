...
def EAD_Discounting_Cost(gov, resList, calLength, decLength, totalLength):
    ...
    benefit_cost_year['EAD_Reduction_SR'] = []
    benefit_cost_year['EAD_Reduction_FS'] = []
    benefit_cost_year['EAD_Reduction_OS'] = []
    ...
    for i in range(totalLength):
        ...
        EAD_Reduction_SR, EAD_Reduction_FS, EAD_Reduction_OS = 0, 0, 0
        ...
        if i < calLength:
            for res in resList:
                ...
                if i < res.selfMoveYear:
                    ...
                    Self_EAD += 0
                if i == res.selfMoveYear:
                    EAD_Reduction_SR += sum([discount(res.ead[j], gov.disRate, j - i) for j in range(i, totalLength)]) / ((1 + gov.disRate) ** i)
                ...
                if i == res.motiMoveYear:
                    ...
                    Moti_TC += (res.replacementcost * gov.subPercent + res.replacementcost + res.relocationcost) / (
                                (1 + gov.disRate) ** i)
                    if res.selfMoveYear < calLength:
                        EAD_Reduction_FS += sum([discount(res.ead[j], gov.disRate, j - i) for j in range(i, res.selfMoveYear)]) / ((1 + gov.disRate) ** i)
                    else:
                        EAD_Reduction_FS += sum([discount(res.ead[j], gov.disRate, j - i) for j in range(i, totalLength)]) / ((1 + gov.disRate) ** i)
                ...
                if i == res.optmotiMoveYear:
                    ...
                    Opt_Moti_TC += (res.Subsidyneeded[i] + res.replacementcost + res.relocationcost) / (
                                (1 + gov.disRate) ** i)
                    if res.selfMoveYear < calLength:
                        EAD_Reduction_OS += sum([discount(res.ead[j], gov.disRate, j - i) for j in range(i, res.selfMoveYear)]) / ((1 + gov.disRate) ** i)
                    else:
                        EAD_Reduction_OS += sum([discount(res.ead[j], gov.disRate, j - i) for j in range(i, totalLength)]) / ((1 + gov.disRate) ** i)
        else:
            for res in resList:
                ## calculate the benefit and cost of self-move
                if i < res.selfMoveYear:
        ...
        benefit_cost_year['EAD_Reduction_SR'].append(EAD_Reduction_SR)
        ...
        benefit_cost_year['EAD_Reduction_FS'].append(EAD_Reduction_FS)
        benefit_cost_year['EAD_Reduction_OS'].append(EAD_Reduction_OS)

    benefit_cost_result = pd.DataFrame(benefit_cost_year)
    return benefit_cost_result


def EAD_WO_Discounting_Cost(gov, resList, calLength, decLength, totalLength):
    benefit_cost_year = {}
    benefit_cost_year['Year'] = []
    benefit_cost_year['Self_EAD'] = []
    benefit_cost_year['Self_Subsidy'] = []
        ...
        if i < calLength:
            for res in resList:
                ...
                if i < res.selfMoveYear:
                    ...
                    Self_EAD += 0
                if i == res.selfMoveYear:
                    EAD_Reduction_SR += sum([res.ead[j] for j in range(i, totalLength)])
                # since under the self-move mode, the government will not provide any subsidy but bear the flood loss, the subsidy and totalcost will be 0
                ## calculate the benefit and cost of fixed-subsidy percentage relocation
                if i < res.motiMoveYear:
                    Moti_EAD += res.ead[i]
                    ...
                    Moti_EAD += 0
                if i == res.motiMoveYear:
                    Moti_Subsidy += res.replacementcost * gov.subPercent
                    Moti_TC += res.replacementcost * gov.subPercent + res.replacementcost + res.relocationcost
                    #                print(res.ead[i:(i + decLength)])
                    if res.selfMoveYear < calLength:
                        EAD_Reduction_FS += sum([res.ead[j] for j in range(i, res.selfMoveYear)])
                    else:
                        EAD_Reduction_FS += sum([res.ead[j] for j in range(i, totalLength)])
                ## calculate the benefit and cost of individually optimized subsidized relocation
                if i < res.optmotiMoveYear:
                ...
                if i == res.optmotiMoveYear:
                    ...
                    Opt_Moti_TC += res.Subsidyneeded[i] + res.replacementcost + res.relocationcost
                    if res.selfMoveYear < calLength:
                        EAD_Reduction_OS += sum([res.ead[j] for j in range(i, res.selfMoveYear)])
                    else:
                        EAD_Reduction_OS += sum([res.ead[j] for j in range(i, totalLength)])
        else:
            for res in resList:
                ## calculate the benefit and cost of self-move
                if i < res.selfMoveYear:
...
def analysis_mhi(resList, mhi_list, mode, subPercent, calLength):
    ...
    if mode == 'Opt':
        for mhi in mhi_list:
            ...
            for res in resList:
                if res.mhi_ratio == mhi:
                    ...
                    if res.optimotiFlag == 1:
                        ...
                        mhi_result[mhi]['Total_Cost'] += res.Subsidyneeded[
                                                             res.optmotiMoveYear] + res.replacementcost + res.relocationcost
            ...
            mhi_result[mhi]['Avg_Subsidy_Amount'] = mhi_result[mhi]['Total_Subsidy_Amount'] / (
                        mhi_result[mhi]['Total_Relocation_Num'] + 1)
